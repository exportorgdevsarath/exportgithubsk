...
public class EtableBaseService<T extends EtableBase> extends BaseBusinessLogic<T>
		implements IEtableBaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(EtableBaseService.class.getName());
	...
	@Autowired
	private  EtablePerimeterBaseImpl etablePerimeterBaseImpl;
	
	
	public EtableBaseService(Class<T> modelClass) {
		super(modelClass);
		...
		registerModelTransformer(PersistenceType.ANALYTICAL, new EtableAnalyticalTransformer());
	}
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("Etable", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("Etable", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("Etable", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return etablePerimeterBaseImpl;
	}
}